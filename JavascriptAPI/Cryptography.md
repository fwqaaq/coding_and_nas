...
2. 对第一个块 M1，先与初始化向量 IV 进行异或，再使用 AES 加密，得到密文 C1
3. 对第二块 M2，先与 C1 进行异或，再加密，得到 C2
...
摘要（也称为单向散列函数）是由散列函数从完整消息生成的更小值。理想情况下，摘要是**可快速计算**、**不可逆**且**不可预测**的，因此可用于表明是否有人篡改了给定的消息。**但是无法辨别伪装**。例如 bob 伪装成 alan 向 marry 发送了消息和摘要。
