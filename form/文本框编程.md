...
* `:checked`:表示任何处于选中状态的(`<input type="radio">`),checkout(`<input type="checkout">`)或者\<select>元素中的option HTML元素
* `:placeholder-shown`:CSS伪类在\<input>或\<textarea>元素显示`placeholder text`时生效
